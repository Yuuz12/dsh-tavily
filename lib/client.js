window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ---------------------------------------------------------------------
		// 样式：全部走 --dsw-alias-* 主题令牌（明暗自适应），无裸色值；
		// 语义色仅 success/warn/error 三类，带令牌回退。
		// 卡片骨架对齐内置 PluginCard：li + 头部按钮 + body。
		// ---------------------------------------------------------------------
		const CSS = [
			...
			'.tvly-body{border-top:1px solid var(--dsw-alias-border-l2);margin:0 16px;padding:4px 0 10px;display:flex;flex-direction:column;gap:12px;color:var(--dsw-alias-label-primary);font-size:13px}',

			'.tvly-status{display:flex;align-items:flex-start;gap:8px;margin:0;padding:8px 10px;border-radius:8px;background:var(--dsw-alias-bg-module-platform);line-height:1.55;font-size:12px;color:var(--dsw-alias-label-secondary)}',
			'.tvly-dot{flex:none;width:8px;height:8px;border-radius:50%;margin-top:4px;background:var(--dsw-alias-label-faint,#9c9c9c)}',

			'.tvly-field{display:flex;flex-direction:column;gap:6px;padding:10px 0;min-width:0}',
			'.tvly-field + .tvly-field{border-top:1px solid var(--dsw-alias-border-l2)}',
			'.tvly-row{display:flex;align-items:center;gap:10px;flex-wrap:wrap}',
			...
			// 开关：轨道用模块底色，选中换品牌色；旋钮用主文字色（暗色下为浅色，天然可见）
			...
			'.tvly-switch input:checked + .tvly-switch-track::after{transform:translateX(16px);background:#fff}',
			...
			'.tvly-input,.tvly-select{box-sizing:border-box;height:32px;padding:0 10px;font:inherit;font-size:13px;line-height:1.5;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;transition:border-color .16s}',
			'.tvly-input:focus-visible,.tvly-select:focus-visible{outline:none;border-color:var(--dsw-alias-brand-primary)}',
			'.tvly-input:disabled,.tvly-select:disabled{color:var(--dsw-alias-label-tertiary);cursor:default}',
			'.tvly-input::placeholder{color:var(--dsw-alias-label-faint,#9c9c9c)}',

			'.tvly-btn{appearance:none;font:inherit;font-size:12px;line-height:1.5;cursor:pointer;padding:5px 12px;border-radius:8px;border:1px solid var(--dsw-alias-border-l2);background:none;color:var(--dsw-alias-label-secondary);transition:color .16s,border-color .16s,opacity .16s}',
			...
			'.tvly-badge-ok{color:var(--dsw-alias-state-success-primary,#1a7f37)}',
			...
			'.tvly-key{display:flex;flex-direction:column;gap:8px;padding:10px 12px;border:1px solid var(--dsw-alias-border-l2);border-radius:8px}',
			...
			'.tvly-labelinput{width:120px;height:26px;padding:0 8px;font:inherit;font-size:12px;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l2);border-radius:6px}',
			'.tvly-spacer{flex:1}',
			...
			'.tvly-msg{margin:0;font-size:12px;line-height:1.5;color:var(--dsw-alias-state-success-primary,#1a7f37)}',
			'.tvly-err{margin:0;font-size:12px;line-height:1.5;color:var(--dsw-alias-label-error,#d1242f);white-space:pre-wrap}',
			'.tvly-hint{margin:0;padding-top:10px;border-top:1px solid var(--dsw-alias-border-l2);font-size:12px;line-height:1.7;color:var(--dsw-alias-label-tertiary)}',
			'.tvly-hint code{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:11px}',
			'.tvly-section-gap{display:flex;flex-direction:column;gap:12px;max-width:720px}',
		].join('\n');
		...
		const ZH = {
			...
			statusNoKeys: '开关已打开，但还没有可用密钥——当前仍由官方搜索兜底。请先添加至少一个 API Key。',
		...
		const EN = {
			...
			statusNoKeys: 'Switch is on but no usable key — official search still serves. Add an API key first.',
			...
			unknownBalance: 'Unknown (refresh balances)',
			staleData: 'possibly stale',
			cooling: 'cooling',
			...
			settingsUnavailable: '(warning: settings service unavailable; the switch cannot persist.)',
			connectFail: 'Cannot reach the dsh-tavily host endpoint: ',
			connectFailSuffix: '. Install the plugin and restart DSH.',
		}
		...
		/**
		 * 共享控制器：持有服务端状态快照 + 少量 UI 态，向两个入口
		 * （keyed 卡片 / 独立设置分区）提供同一份 store 与动作集。
		 */
		class TavilyController {
			...
			async load() {
				try {
					...
					if (r && r.ok) { this.data = r.state; this.ui.error = ''; }
					else if (!this.data) this.ui.error = (r && r.error) || 'HTTP error';
				} catch (e) {
					if (!this.data) this.ui.error = (e && e.message ? e.message : String(e));
				}
				this.publish();
		...
		/** 手写订阅读取：给不经过 slots hooks 注入的入口（独立设置分区）用。 */
		function useController(controller) {
			const [snap, setSnap] = react.useState(controller.getSnapshot());
			react.useEffect(() => {
				const un = controller.subscribe(() => setSnap(controller.getSnapshot()));
		...
		function BalanceBar({ usage, stale }) {
			if (!usage || (usage.usage === null && usage.limit === null)) {
			...
			return react.createElement(react.Fragment, null,
				react.createElement('div', { className: 'tvly-bar', title: fmtInt(used) + (limit != null ? ' / ' + fmtInt(limit) : '') },
					react.createElement('i', { style: { width: pct + '%', background: color } })
				),
				react.createElement('span', { className: 'tvly-baltext' },
					fmtInt(used) + (limit != null ? ' / ' + fmtInt(limit) : '') +
					' · 剩余 ' + (limit == null ? '∞' : fmtInt(limit - used)) +
					(usage.plan ? ' · ' + usage.plan : '')
				),
				stale && limit != null && react.createElement('span', { className: 'tvly-badge' }, ZH.staleData)
			);
		}

		/**
		 * 卡片主体（内容层）。props：
		 *  - state: 控制器快照（ready/data/busy/message/error）
		 *  - ctrl : 控制器实例（动作方法）
		 *  - t    : 可选 locale 函数
		 */
		function TavilyCardBody({ state, ctrl, t }) {
			...
			function onLabelSave(id) {
				...
				ctrl.updateKey(id, { label: draft.trim() });
			}
			...
			return react.createElement(react.Fragment, null,
				...
				react.createElement('div', { className: 'tvly-field' },
					react.createElement('div', { className: 'tvly-row' },
				...
				react.createElement('div', { className: 'tvly-field' },
					react.createElement('div', { className: 'tvly-row' },
						react.createElement('span', { className: 'tvly-sub' }, T('strategyLabel')),
						...
						react.createElement('span', { className: 'tvly-sub' }, T('depthLabel')),
						react.createElement('select', {
							className: 'tvly-select', value: cfg.searchDepth || 'basic', disabled: busy,
							onChange: (e) => ctrl.patchConfig({ searchDepth: e.target.value }),
						},
							react.createElement('option', { value: 'basic' }, 'basic · 1cr'),
							react.createElement('option', { value: 'advanced' }, 'advanced · 2cr'),
							react.createElement('option', { value: 'fast' }, 'fast'),
							react.createElement('option', { value: 'ultra-fast' }, 'ultra-fast')
						),
						react.createElement('span', { className: 'tvly-sub' }, T('topicLabel')),
						react.createElement('select', {
						...
						react.createElement('span', { className: 'tvly-sub' }, T('maxResultsLabel')),
						react.createElement('input', {
							className: 'tvly-input', style: { width: 64 }, type: 'number', min: 1, max: 20,
						...
						react.createElement('span', { className: 'tvly-sub' }, T('answerLabel')),
						react.createElement('label', { className: 'tvly-switch' },
							react.createElement('input', {
				...
				react.createElement('ul', { className: 'tvly-keys' },
					keys.map((k, index) => {
						const st = stats[k.id] || {};
						return react.createElement('li', { className: 'tvly-key', key: k.id, 'data-disabled': k.disabled ? 'true' : 'false' },
							react.createElement('div', { className: 'tvly-key-top' },
								react.createElement('span', { className: 'tvly-order' },
									react.createElement('button', {
										className: 'tvly-arrow', disabled: busy || index === 0, title: '↑',
										onClick: () => ctrl.moveKey(k.id, -1),
									}, '▲'),
									react.createElement('button', {
										className: 'tvly-arrow', disabled: busy || index === keys.length - 1, title: '↓',
										onClick: () => ctrl.moveKey(k.id, 1),
									}, '▼')
								),
								...
								react.createElement('input', {
									...
									value: labelDrafts[k.id] !== undefined ? labelDrafts[k.id] : (k.label || ''),
									placeholder: '备注名', maxLength: 40, disabled: busy,
									onChange: (e) => setLabelDrafts((prev) => ({ ...prev, [k.id]: e.target.value })),
									onBlur: () => onLabelSave(k.id),
									onKeyDown: (e) => { if (e.key === 'Enter') e.target.blur(); },
								}),
								k.hasWarningPrefix && react.createElement('span', { className: 'tvly-badge tvly-badge-warn', title: 'tvly-' }, T('formatWarn')),
								st.cooling && react.createElement('span', { className: 'tvly-badge tvly-badge-warn' }, T('cooling')),
								k.disabled && react.createElement('span', { className: 'tvly-badge' }, T('disabledBadge')),
								react.createElement('span', { className: 'tvly-spacer' }),
								react.createElement('button', { className: 'tvly-btn', disabled: busy, onClick: () => ctrl.testKey(k.id) }, T('test')),
								react.createElement('button', { className: 'tvly-btn', disabled: busy, onClick: () => ctrl.updateKey(k.id, { disabled: !k.disabled }) }, k.disabled ? T('enable') : T('disable')),
								react.createElement('button', {
									className: 'tvly-btn tvly-btn-danger', disabled: busy,
									onClick: () => { if (window.confirm(T('removeConfirm'))) ctrl.removeKey(k.id); },
								}, T('remove'))
							),
							react.createElement('div', { className: 'tvly-balrow' },
								react.createElement(BalanceBar, { usage: k.usage, stale: k.stale })
							),
							react.createElement('div', { className: 'tvly-stats' },
								react.createElement('span', null, T('calls'), ' ', react.createElement('b', null, fmtInt(st.requests))),
								react.createElement('span', null, T('successN'), ' ', react.createElement('b', null, fmtInt(st.success))),
								react.createElement('span', null, T('failedN'), ' ', react.createElement('b', null, fmtInt(st.failed))),
								react.createElement('span', null, T('credits'), ' ', react.createElement('b', null, fmtInt(st.creditsUsed))),
								st.lastLatencyMs != null && react.createElement('span', null, T('lastLatency'), ' ', react.createElement('b', null, st.lastLatencyMs + 'ms')),
								st.lastUsedAt && react.createElement('span', null, T('lastUsed'), ' ', new Date(st.lastUsedAt).toLocaleString())
							),
							st.lastError && react.createElement('p', { className: 'tvly-lasterr', title: st.lastError }, T('lastErrorPrefix') + st.lastError)
						);
					}),
				...
				react.createElement('div', { className: 'tvly-row' },
					react.createElement('input', {
						className: 'tvly-input', style: { flex: '1 1 220px' }, type: 'password', autoComplete: 'off',
						placeholder: T('keyPlaceholder'), value: newKey, disabled: busy,
						onChange: (e) => setNewKey(e.target.value),
					}),
					react.createElement('input', {
						className: 'tvly-input', style: { flex: '0 1 150px' }, type: 'text',
						placeholder: T('labelPlaceholder'), value: newLabel, maxLength: 40, disabled: busy,
						onChange: (e) => setNewLabel(e.target.value),
					}),
					react.createElement('button', {
						className: 'tvly-btn tvly-btn-solid', disabled: busy || newKey.trim().length < 8,
						onClick: () => { ctrl.addKey(newKey, newLabel).then((r) => { if (r && r.ok) { setNewKey(''); setNewLabel(''); } }); },
					}, T('addKey')),
					react.createElement('button', {
						className: 'tvly-btn', disabled: busy || keys.length === 0,
						onClick: () => ctrl.refreshUsage(),
					}, T('refreshUsage'))
				),
				...
				data && data.dataPath && react.createElement('p', { className: 'tvly-hint' },
					'密钥仅存本机，界面只显示脱敏形式；「测试」会真实发起一次 basic 搜索（消耗 1 积分）。Key 存储：',
		...
		/**
		 * 入口一（主）：插件配置标签页的 keyed 卡片。
		 * 结构对齐内置 PluginCard：li 折叠卡；数据经 slots hooks 注入
		 * （inject 返回 hooks.{handle: store} → 组件收到 props.use<Handle>）。
		 */
		...
		/**
		 * 入口二（兜底）：独立设置分区「Tavily 搜索」，始终出现在设置左侧导航，
		 * 不依赖 keyed 派发。数据经闭包内控制器 + 手写订阅读取。
		 */
		function makeSectionComponent(ctrl) {
			return function TavilySection(props) {
				const t = props && props.t;
				const snap = useController(ctrl);
				const triedLoad = react.useRef(false);
				react.useEffect(() => {
					if (!triedLoad.current) { triedLoad.current = true; if (!snap.ready) ctrl.load(); }
				}, []);
				if (!snap.ready) {
					return react.createElement('div', { className: 'tvly-section-gap' },
						react.createElement('p', { className: 'tvly-sub' }, snap.error || 'Loading…'));
				}
				return react.createElement('div', { className: 'tvly-section-gap' },
					react.createElement('h2', {
						style: { margin: 0, fontSize: 18, fontWeight: 600, lineHeight: 1.4, color: 'var(--dsw-alias-label-primary)' },
					}, (t ? t('title') : null) || ZH.title),
					react.createElement(TavilyCardBody, { state: snap, ctrl, t })
				);
			};
		}

		function apply(ctx) {
			// 样式注入（幂等：先移除旧 tag 再插入）
			ctx.effect(() => {
			...
			// —— 入口二：独立设置分区（设置左导航「Tavily 搜索」），兜底 ——
			ctx.slots.inject('settings.section', () => ctx.slots.register({
				name: 'settings.section',
				id: 'tavily-search',
				order: 26,
				label: 'Tavily 搜索',
			}, makeSectionComponent(ctrl)));
		}
